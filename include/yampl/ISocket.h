...
class ISocket{
  public:
     ...
     * message is sent to is identified with a discriminator.
     *
     * @param buffer the message that contains the data to send
     * @param size the size of the message to send
     * @param discriminator the identifier of the peer to which the message has 
     ...
     *        a ServerSocket, and the only peer in the case of a ClientSocket
     * @param hint the pointer is passed to the deallocator and can be used to 
     *        implement complex deallocation policies
     *
     * @throw UnroutableException if the message can't be sent to the specified 
     *        discriminator (e.g. peer might be temporarly unavailable)
     ...
     * the sending peer is returned through the discriminator parameter.
     *
     * @param buffer pointer to the buffer in which the receiving message will 
     *        be stored; if buffer points to a NULL-pointer then the implementation 
     *        will allocate a buffer for the message to be received
     * @param size the size of the buffer; the size is ignored if buffer points 
     *        to a NULL-pointer
     * @param discriminator the identifier of the peer from which the data has 
     *        been received
